...
const reactions = ['👍', '❤️', '😂', '😮', '😢', '😡', '🎉', '🔥', '👏', '💯'];
const gestures = ['👋', '🤝', '👊', '✌️', '🤟', '🖖', '🙏', '💪', '🫶', '👌'];
const badges = ['🌟 Новичок', '🔥 Активный', '💬 Болтун', '👑 Лидер', '🎨 Дизайнер', '🤖 Бот', '💎 VIP', '⭐ Звезда'];
const statuses = ['В сети', 'Отошёл', 'Не беспокоить', 'За работой', 'Сплю', 'В пути', 'За столом', 'На встрече'];
const stickerPacks = ['😀 Эмоции', '🐱 Животные', '🍕 Еда', '🎮 Игры', '🎬 Кино', '⚽ Спорт', '🌈 Радуга', '🔥 Огонь'];
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .reaction-btn { background: none; border: none; font-size: 16px; cursor: pointer; padding: 2px 5px; border-radius: 10px; }
        .reaction-picker {
            position: absolute;
            bottom: 100%;
            left: 0;
        ...
        .message-content { padding: 8px 14px; border-radius: 18px; position: relative; }
        ...
        .attach-btn, .voice-record-btn, .video-record-btn, .sticker-btn, .reaction-open-btn { background: #2a2a3e !important; }
...
function playAudioWithControls(btn) {
...
function addMessage(msg) {
    ...
    div.innerHTML = '<div class="message-avatar" onclick="viewUserProfile(\\'' + msg.from + '\\')">👤</div><div class="message-bubble"><div class="message-content"><div class="message-username" onclick="viewUserProfile(\\'' + msg.from + '\\')">' + escapeHtml(msg.from) + '</div><div class="message-text">' + escapeHtml(msg.text) + '</div><div class="message-time">' + (msg.time || getLocalTime()) + '</div></div></div>';
    document.getElementById('messages').appendChild(div);
}
function addVoiceMessage(data) {
    const div = document.createElement('div'); div.className = 'message';
    if (data.from === currentUser) div.className += ' my-message';
    div.innerHTML = '<div class="message-avatar" onclick="viewUserProfile(\\'' + data.from + '\\')">👤</div><div class="message-bubble"><div class="message-content"><div class="message-username" onclick="viewUserProfile(\\'' + data.from + '\\')">' + escapeHtml(data.from) + '</div><div class="voice-message"><button onclick="playAudioWithControls(this)" data-audio="' + data.audio + '">▶️</button><span>Голосовое</span></div><div class="message-time">' + (data.time || getLocalTime()) + '</div></div></div>';
    document.getElementById('messages').appendChild(div);
}
function addVideoMessage(data) {
    const div = document.createElement('div'); div.className = 'message';
    if (data.from === currentUser) div.className += ' my-message';
    div.innerHTML = '<div class="message-avatar" onclick="viewUserProfile(\\'' + data.from + '\\')">👤</div><div class="message-bubble"><div class="message-content"><div class="message-username" onclick="viewUserProfile(\\'' + data.from + '\\')">' + escapeHtml(data.from) + '</div><video class="video-circle" controls autoplay loop src="' + data.video + '"></video><div class="message-time">' + (data.time || getLocalTime()) + '</div></div></div>';
    document.getElementById('messages').appendChild(div);
}
function addFileMessage(data) {
    ...
    div.innerHTML = '<div class="message-avatar" onclick="viewUserProfile(\\'' + data.from + '\\')">👤</div><div class="message-bubble"><div class="message-content"><div class="message-username" onclick="viewUserProfile(\\'' + data.from + '\\')">' + escapeHtml(data.from) + '</div><div class="file-attachment"><span>' + icon + '</span><a href="' + data.fileData + '" download="' + data.fileName + '">' + data.fileName + '</a></div><div class="message-time">' + (data.time || getLocalTime()) + '</div></div></div>';
    document.getElementById('messages').appendChild(div);
}
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log('║  ✅ 200+ фишек (стикеры, реакции)     ║');
    console.log('╚════════════════════════════════════════╝\n');
});
