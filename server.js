...
function loadData() {
    try {
        if (fs.existsSync(DATA_FILE)) {
            return JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));const express = require('express');
const http = require('http');
const socketIo = require('socket.io');
const fs = require('fs');
const path = require('path');
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        /* КАСТОМНЫЕ СТИЛИ ДЛЯ ЧАТОВ И ПРОФИЛЯ */
        .custom-bg { background-size: cover; background-position: center; background-repeat: no-repeat; }
        .custom-chat-bg .messages-area { background-size: cover; background-position: center; }
...
<div id="profileModal" class="modal" style="display:none">
    <div class="modal-content">
        ...
        <div class="profile-field"><label>Цвет сообщений</label><input type="color" id="editMsgColor" value="#667eea"></div>
        <div class="profile-field"><label>Фон чата</label><input type="text" id="editChatBg" placeholder="URL фона или цвет"></div>
...
<div id="customModal" class="modal" style="display:none">
    <div class="modal-content">
        <div class="modal-header"><h3>Кастомизация чата</h3><button class="close-modal" onclick="closeCustomModal()">✕</button></div>
        <div class="profile-field"><label>Цвет моих сообщений</label><input type="color" id="customMyMsgColor" value="#667eea" onchange="applyCustomStyle()"></div>
        <div class="profile-field"><label>Цвет сообщений собеседника</label><input type="color" id="customOtherMsgColor" value="#2a2a3e" onchange="applyCustomStyle()"></div>
        <div class="profile-field"><label>Фон чата (URL или цвет)</label><input type="text" id="customChatBg" placeholder="#0a0a0a или https://example.com/bg.jpg" onchange="applyCustomStyle()"></div>
        <div class="profile-field"><label>Размер шрифта</label><select id="customFontSize" onchange="applyCustomStyle()">
            <option value="12px">Маленький</option>
            <option value="14px">Средний</option>
            <option value="16px">Большой</option>
            <option value="18px">Очень большой</option>
        </select></div>
        <div class="profile-field"><label>Скругление сообщений</label><select id="customBorderRadius" onchange="applyCustomStyle()">
            <option value="12px">Маленькое</option>
            <option value="18px">Среднее</option>
            <option value="25px">Большое</option>
            <option value="35px">Очень большое</option>
        </select></div>
...
function applyCustomStyle() {
    const myMsgColor = document.getElementById('customMyMsgColor')?.value || '#667eea';
    const otherMsgColor = document.getElementById('customOtherMsgColor')?.value || '#2a2a3e';
    const chatBg = document.getElementById('customChatBg')?.value || '';
    const fontSize = document.getElementById('customFontSize')?.value || '14px';
    const borderRadius = document.getElementById('customBorderRadius')?.value || '18px';
    ...
    style.innerHTML = `
        .message.my-message .message-content { background: ${myMsgColor} !important; }
    ...
    `;
    if (chatBg) {
        document.querySelector('.messages-area').style.background = chatBg;
        document.querySelector('.messages-area').style.backgroundSize = 'cover';
    } else {
        document.querySelector('.messages-area').style.background = '';
    }
}
function saveCustomStyle() {
    ...
    socket.emit('save custom style', data, (res) => {
        if (res.success) showNotification('Кастомизация', 'Стили сохранены');
    });
    closeCustomModal();
}
...
function openProfileModal() {
    ...
    document.getElementById('editMsgColor').value = currentUserData?.myMsgColor || '#667eea';
    document.getElementById('editChatBg').value = currentUserData?.chatBg || '';
...
function saveProfile() {
    const data = { 
        ...
        myMsgColor: document.getElementById('editMsgColor').value,
        chatBg: document.getElementById('editChatBg').value,
...
function login() {
    ...
    socket.emit('login', { username, password }, (res) => {
        if (res.success) {
            ...
            if (res.userData.myMsgColor || res.userData.chatBg) {
                document.getElementById('customMyMsgColor').value = res.userData.myMsgColor || '#667eea';
                document.getElementById('customOtherMsgColor').value = res.userData.otherMsgColor || '#2a2a3e';
                document.getElementById('customChatBg').value = res.userData.chatBg || '';
                document.getElementById('customFontSize').value = res.userData.fontSize || '14px';
                document.getElementById('customBorderRadius').value = res.userData.borderRadius || '18px';
                applyCustomStyle();
            }
...
io.on('connection', (socket) => {
    ...
    socket.on('save custom style', (data, callback) => {
        if (users[data.login]) {
    ...
    socket.on('update profile', (data, callback) => {
        if (users[data.login]) {
            ...
            if (data.myMsgColor !== undefined) users[data.login].myMsgColor = data.myMsgColor;
            if (data.chatBg !== undefined) users[data.login].chatBg = data.chatBg;
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log('✅ Видеокружки теперь воспроизводятся (controls autoplay loop)');
    console.log('✅ Кастомизация: цвет сообщений, фон чата, размер шрифта, скругление');
